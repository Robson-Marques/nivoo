  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        ...
        // Se a tabela não existir, não mascarar o problema
        if ((error as any)?.code === 'PGRST205') {
          throw error;
        }

        // Retorna array vazio em caso de erro de RLS ou permissão
        return [] as Coupon[];
      }
      console.log('Cupons carregados:', data, `Total: ${data?.length || 0}`);
  ...
  return useMutation({
    ...
    onError: (error) => {
      ...
      if (anyErr?.code === 'PGRST205') {
        toast.error('Erro ao criar cupom: tabela "coupons" não existe no Supabase');
        return;
      }
...
export function useUpdateCoupon(couponId: string) {
  ...
  return useMutation({
    ...
    onError: (error) => {
      const anyErr = error as any;
      if (anyErr?.code === 'PGRST205') {
        toast.error('Erro ao atualizar cupom: tabela "coupons" não existe no Supabase');
        return;
      }
...
export function usePromotions() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      console.log('Promoções carregadas:', data, `Total: ${data?.length || 0}`);
